...
std::vector<std::vector<cv::vec2r> > read_pattern_results(const char* path, unsigned& im_w, unsigned& im_h) {
    ...
    while (std::getline(stream, line)) {
        ...
        for (auto& v : split_string(line, ",")) {
            ...
            if (vec.size() == 2) {
                p.push_back(cv::vec2r(static_cast<real_t>(std::atof(vec[0].c_str())),
                                      static_cast<real_t>(std::atof(vec[1].c_str()))));
            }
        }
    }
...
bool read_zhang_data(const std::string& folderpath,
...
bool read_custom_data(const std::string& filepath,
                      std::vector<std::vector<cv::vec2r> >& image_points,
                      std::vector<cv::vec3r>& model_points,
                      unsigned& im_w,
                      unsigned& im_h,
    ...
    model_points = calculate_object_points(12, 19, model_size);
...
int main(int argc, char** argv) {
    ...
    if (pattern_file == "zhang") {
        read_zhang_data("calib_data/zhang_data", image_points_nrm, model_points, im_w, im_h);
    } else {
