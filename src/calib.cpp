...
bool extract_intrinsics_from_B(const cv::matrixr& B,
                               real_t& u0,
                               real_t& v0,
                               real_t& lambda,
                               real_t& alpha,
...
cv::vectorr reproject_point(const cv::vectorr& world_ptn,
                            const cv::vec2r& norm_ptn,
                            const cv::matrixr& A,
                            const cv::matrixr& K,
                            const cv::vectorr& k) {
    ...
    const real_t u0 = A(0, 2);
    const real_t v0 = A(1, 2);
    const real_t u = pp_vec[0];
    const real_t v = pp_vec[1];
    const real_t x = norm_ptn[0];
    const real_t y = norm_ptn[1];
    const real_t xy2 = x * x + y * y;

    real_t u_, v_;

    if (k.length() == 2) {
        const real_t dist_intensity = k[0] * xy2 + k[1] * (xy2 * xy2);
        u_ = u + (u - u0) * dist_intensity;
        v_ = v + (v - v0) * dist_intensity;
    } else if (k.length() == 4) {
        u_ = u + (u - u0) * (k[0] * xy2 + k[1] * (xy2 * xy2) + k[2] * (xy2 * xy2 * xy2) + k[3] * (xy2 * xy2 * xy2 * xy2));
        v_ = v + (v - v0) * (k[0] * xy2 + k[1] * (xy2 * xy2) + k[2] * (xy2 * xy2 * xy2) + k[3] * (xy2 * xy2 * xy2 * xy2));
    } else if (k.length() == 8) {
        ...
        throw std::runtime_error("Invalid distortion parameter vector configuration - should be 2, 4, or 8 length.");
    }
...
real_t calc_reprojection(const cv::matrixr& A, const cv::matrixr& K, const std::vector<cv::vec3r>& model_pts,
                         const std::vector<cv::vec2r>& image_pts, const std::vector<cv::vec2r>& image_pts_nrm,
    ...
    for (unsigned i = 0; i < m; i++) {
        ...
        auto proj_ptn = reproject_point(model, image_pts_nrm[i], A, K, k);
...
cv::vectorr compute_distortion(const std::vector<std::vector<cv::vec2r> >& image_pts,
    ...
    for (unsigned b = 0; b < image_pts.size(); ++b) {
        for (unsigned i = 0; i < n_pts; i++) {
            ...
            const auto id = b * n_pts * 2 + i * 2;

            D(id, 0) = (u_uo) * (x2_y2);
            D(id, 1) = (u_uo) * (x2_y2) * (x2_y2);

            D(id + 1, 0) = (v_vo) * (x2_y2);
            D(id + 1, 1) = (v_vo) * (x2_y2) * (x2_y2);

            d(id, 0) = image_pts[b][i][0] - image_pts_proj[b][i][0];
            d(id + 1, 0) = image_pts[b][i][1] - image_pts_proj[b][i][1];
        }
    }
