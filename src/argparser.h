...
public:
	...
	/// Fetch string argument, assuming such argument is given by a requested key. If there's no such argument, std::invalid_argument is thrown.
	std::string fetch_string(const std::string& key);
	/// Fetch integer argument, assuming such argument is given by a requested key. If there's no such argument, std::invalid_argument is thrown.
	int fetch_int(const std::string& key);
	/// Fetch doubleiing point argument, assuming such argument is given by a requested key. If there's no such argument, std::invalid_argument is thrown.
